...
proj.means = proj.means %>%
  mutate(Party = fct_relevel(Region, "Non-Southern", "Southern"))
levels(proj.means$Region)

# to add participants' ratings
subjmeans = d.neutralCCs %>%
  group_by(workerid,predicate,Region) %>%
...
subjmeans
subjmeans$Region<- as.factor(subjmeans$Region)

# change factor levels for prior_type for plotting
subjmeans = subjmeans %>%
  mutate(Party = fct_relevel(Region,"Non-Southern", "Southern"))
levels(subjmeans$Region)

levels(proj.means$Region)
#[1] "Non-Southern" "Southern"

ggplot(proj.means, aes(x=predicate, y=Mean, color=Region,fill=Region,shape=Region)) + 
  theme(legend.position = "top", legend.text=element_text(size=12)) +
  geom_point(data=subjmeans,aes(fill=Region,color=Region),shape=21,alpha=.08) +
...
projection_socialInfo_political<-
  ...
  labs(fill="Social Info")+
  ylim(0,1)+
  theme(legend.position = c(.5,.85),axis.text=element_text(size=8),axis.title=element_text(size=8,face="bold"),legend.title=element_blank(),legend.text=element_text(size=8),aspect.ratio=1,legend.margin=margin(r=2,l=2,t=-2,b=2),legend.key.size = unit(0.2, "cm"))

projection_socialInfo_political
ggsave(projection_socialInfo_political,filename="../graphs/projection~socialInfo_Region.png")
...
# change factor levels for Party for plotting
proj.means = proj.means %>%
  mutate(Region = fct_relevel(Region, "Non-Southern", "Southern"))
levels(proj.means$Region)
...
levels(subjmeans$predicate)
subjmeans
subjmeans$Party<- as.factor(subjmeans$Region)

# change factor levels for prior_type for plotting
subjmeans = subjmeans %>%
  mutate(Party = fct_relevel(Party,"Non-Southern", "Southern"))
...
ggplot(allCCs) +
  ...
  scale_color_manual(values=c("#56B4E9","tomato"),labels=c("Non-Southern","Southern"),name=c("Speaker region")) +
...
ggplot(MCs, aes(x=spBelief_rating, y=projection,color=socialInfo)) +
  ...
  geom_smooth(method="lm",colour="grey50") +
  geom_point(shape=20, size=1, alpha=.3) +
  scale_color_manual(values=c("#56B4E9","tomato"),labels=c("Non-Southern","Southern"),name="Speaker region") +
  xlab("Perceived speaker belief rating") +
  ylab("Certainty rating") +
  theme(legend.position = "bottom", legend.text=element_text(size=12)) +
  guides(colour = guide_legend(override.aes = list(alpha = 1,size=3))) +
...
################################################################################
###H.2.3.c: are Southern speakers rated as more likely to be Republican than non-Southern speakers; vice versa for Democrat ratings?#########################################################
...
SpRepRating_socialInfo<-ggplot(subset(cd,block2=="block2"), aes(x=socialInfo,y=republican_rating,fill=socialInfo),xpd=FALSE) +
  ...
  stat_summary(geom="errorbar",fun.data=mean_cl_boot,position=position_dodge(),width=.5)+
  scale_fill_manual(labels=c("Non-southern speaker","Southern speaker"),values=c("#56B4E9", "tomato1"))+
  xlab("Speaker regional background") +
...
ggsave(SpRepRating_socialInfo ,filename="../graphs/speakerEvaluations/RepRating~region.png")
...
ggsave(SpDemRating_socialInfo ,filename="../graphs/speakerEvaluations/DemRating~region.png")
...
################################################################################
###H.2.3.d.i: are speaker belief ratings for conservative CCs higher when speaker is rated as more likely to be Republican than Democrat; vice versa for liberal CCs#########################################################
...
projection_alignedBeliefs<-ggplot(subset(cd.aligned,(spBelief_rating!="NA" & projection!="NA" & item_type=="critical")))+
...
nrow(subset(cd,spBelief_rating!="NA" & projection!="NA" & item_type=="critical")) #3600
nrow(subset(cd.aligned,spBelief_rating!="NA" & projection!="NA" & item_type=="critical")) #1979
nrow(subset(cd.misaligned,spBelief_rating!="NA" & projection!="NA" & item_type=="critical")) #1621
#1979+1621=3600
...
projection_misalignedBeliefs<-ggplot(subset(cd.misaligned,(spBelief_rating!="NA" & projection!="NA" & item_type=="critical")))+
  geom_smooth(method="lm",aes(x=spBelief_rating,y=projection,color="spBelief_rating")) +
  geom_point(shape=20, size=1, alpha=.3,aes(x=spBelief_rating,y=projection,color="spBelief_rating")) +
  geom_smooth(method="lm",aes(x=participant_beliefs,y=projection,color="participant_beliefs")) +
  geom_point(shape=20, size=1, alpha=.3,aes(x=participant_beliefs,y=projection,color="participant_beliefs")) +
